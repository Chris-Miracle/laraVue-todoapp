    ...
    <div class="w-50">
        <form @submit.prevent="saveData">
            <div class="input-group mb-3 w-100">
                <input v-model="form.title" :class="{'is-invalid' : form.errors.has('title')}" type="text" class="form-control form-control-lg" @keydown="form.errors.clear('title')" name="title" placeholder="What do you wanna do?" aria-label="Recipient's username" aria-describedby="button-addon2">
        ...
        <div class="w-100 todo">
            <div v-for="todo in todos" :key="todo.id" class="w-100 d-flex align-items-center p-3 bg-white border-bottom">
                ...
                <div  class="font-weight-bolder"><span v-if="editmode == false || editmode != todo.id">{{todo.title}}</span><input v-if="editmode == todo.id" class="form-control" style="border: 0px;"   v-model="todo.title" type="text"></div>
